...
ifndef MODULE_NAME
  ...
  ifeq ($(CORE),arp32_1)
    ...
    ifeq ($(SOC), $(filter $(SOC), tda2xx tda2px))
      MISC_LNKCMD_INCLUDE += $(vision_sdk_PATH)/links_fw/src/rtos/bios_app_common/tda2xx/eve1/linker_cmd_eve1.cmd
    endif
  endif
  ifeq ($(CORE),arp32_2)
    PKG_LIST_ARP32_REMOTE = $(foreach COMP,$(COMP_LIST_$(CORE)),$($(COMP)_PKG_LIST))
    CFLAGS_APP_DEFINES += -D_LOCAL_CORE_$(CORE)_
    ifeq ($(SOC), $(filter $(SOC), tda2xx tda2px))
      MISC_LNKCMD_INCLUDE += $(vision_sdk_PATH)/links_fw/src/rtos/bios_app_common/tda2xx/eve2/linker_cmd_eve2.cmd
    endif
  endif
  ifeq ($(CORE),arp32_3)
    PKG_LIST_ARP32_REMOTE = $(foreach COMP,$(COMP_LIST_$(CORE)),$($(COMP)_PKG_LIST))
    CFLAGS_APP_DEFINES += -D_LOCAL_CORE_$(CORE)_
    ifeq ($(SOC),tda2xx)
      MISC_LNKCMD_INCLUDE += $(vision_sdk_PATH)/links_fw/src/rtos/bios_app_common/tda2xx/eve3/linker_cmd_eve3.cmd
    endif
  endif
  ifeq ($(CORE),arp32_4)
    PKG_LIST_ARP32_REMOTE = $(foreach COMP,$(COMP_LIST_$(CORE)),$($(COMP)_PKG_LIST))
    CFLAGS_APP_DEFINES += -D_LOCAL_CORE_$(CORE)_
    ifeq ($(SOC),tda2xx)
      MISC_LNKCMD_INCLUDE += $(vision_sdk_PATH)/links_fw/src/rtos/bios_app_common/tda2xx/eve4/linker_cmd_eve4.cmd
    endif
  endif
